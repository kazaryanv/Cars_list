    ...
    <div class="container">
        ...
        <form action="{{ (isset($cars)) ? route('cars.update', $cars)  : route('cars.store') }}" method="post" enctype="multipart/form-data">
            ...
            <select class="form-control" id="car_brand"  name="car_brand">
                <option>Please select a car brand</option>
                @foreach(\App\Models\Brand::all() as $carList)
                    <option @if((isset($cars)) ? $cars->car_brand : old('car_brand', '')) selected @endif value="{{$carList->car_brand}}">{{$carList->car_brand}}</option>
                @endforeach
            </select>
            <br>
            <select class="form-control" id="car_model"  name="car_model">
                <option>Please select a car model</option>
                @foreach(\App\Models\Brand::all() as $carList)
                    <option @if((isset($cars)) ? $cars->car_model : old('car_model', '')) selected @endif value="{{$carList->car_model}}">{{$carList->car_model}}</option>
                @endforeach
            </select>
            <br>
            <select class="form-control" id="car_years"  name="car_years">
                ...
                @for($i=1971;$i<date('Y');$i++)
                    <option @if((isset($cars)) ? $cars->car_years : old('car_years', '')) selected @endif value="{{$i}}">{{$i}}</option>
                @endfor
            </select>
            <br>
            <select class="form-control" id="car_Engine_capacity"  name="car_Engine_capacity">
                ...
                @for($i=0.5;$i<=7;$i++)
                    <option @if((isset($cars)) ? $cars->car_Engine_capacity : old('car_Engine_capacity', '')) selected @endif value="{{$i}}">{{$i}}L</option>
                @endfor
            </select>
            <br>
            <select class="form-control" id="car_Transmission"  name="car_Transmission">
                ...
                @foreach(['Aftomat','Mexanika'] as $carList)
                    <option @if((isset($cars)) ? $cars->car_Transmission : old('car_Transmission', '')) selected @endif value="{{$carList}}">{{$carList}}</option>
                @endforeach
            </select>
            <br>
